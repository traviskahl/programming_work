...
int main() {
    ...
    for (int startIndex = 0; startIndex < length - 1; startIndex++) {
        // largestIndex is the index of the largest element we've encountered in this iteration
        // Start by assuming the largest element is the first element of this iteration
        int largestIndex = startIndex;

        // Then look for a smaller element in the rest of the array
        for (int currentIndex = startIndex + 1; currentIndex < length; currentIndex++) {
            // If we've found an element that is larger than our previously found smallest
